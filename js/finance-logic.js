...
// antiga).
//
// SALDO (Balance): fluxo Saldo Inicial (opcional, ver FASES) + Depósito →
// Saldo → Aposta → Resultado → Saldo → (domingo) + Bônus → Saldo. Por
// isso o Saldo NÃO é digitado em lugar nenhum (fora do Saldo Inicial de
// uma nova fase) — é sempre CALCULADO. Saldo de plataforma nunca é
// negativo na prática — o piso é sempre R$ 0,00.
//
...
// simplesmente começa a contar a partir daquele momento, com um SALDO
// INICIAL informado manualmente na hora de abrir a fase (obrigatório —
// pensado pra migração da planilha externa: no dia da virada, você
// digita o Saldo real de cada plataforma e o sistema passa a somar os
// movimentos daí pra frente em cima desse valor, sem herdar nenhum
// cálculo da fase anterior). Cada fase (inclusive as já fechadas) fica
// sempre visível e é recalculada AO VIVO a partir dos dados brutos +
// Saldo Inicial daquela fase (nunca um retrato congelado que possa estar
// errado) — ver computePhaseHistory. O Saldo do dia a dia (badge, Total
// da plataforma, Painel Geral) é sempre o da fase ATUAL (a mais recente).
//
...
// criada, ou se a fase foi criada antes dessa funcionalidade existir
// (fases antigas sem o campo `initialBalance` contam como 0, mantendo o
...
// (padrão: agora), com um SALDO INICIAL informado na hora (obrigatório
// na UI — ver buildPhaseControls em ui-finance-panel.js; aqui o valor é
// só travado em zero se vier negativo ou inválido, como qualquer outro
// Saldo do app). A partir dali, computeLiveBalance passa a somar os
// movimentos da fase nova EM CIMA desse Saldo Inicial — nada de fases
// anteriores é herdado além dele. O resultado da fase que acabou de
// fechar continua disponível pra sempre em computePhaseHistory (nunca é
// apagado, só deixa de contar pro Saldo do dia a dia).
export function startNewPhase(platform, date = new Date(), initialBalance = 0) {
...
// O Saldo Inicial dessa fase removida some junto (ela deixa de existir).
...
// Nada de fases anteriores entra aqui além do Saldo Inicial travado no
// momento em que a fase foi aberta — cada fase é independente, de
...
export function computeLiveBalance(platform, refDate = new Date()) {
  ...
  const balance = initialBalance + depositTotal - withdrawalTotal + closedResultBetting + currentWeekResultBetting + bonusTotal;
...
// dos dados brutos + Saldo Inicial daquela fase — nunca um valor
// congelado. Se a plataforma nunca teve nenhuma fase criada, retorna uma
// única "Fase 1" cobrindo a vida inteira dela, com Saldo Inicial R$ 0,00
// (mesmo comportamento de antes dessa funcionalidade existir). Cada fase
// soma Saldo Inicial + Depósito, Saque, Apostado, N° Apostas, Bônus e
// R.B. só dentro do próprio intervalo — igual computeLiveBalance faz pra
// fase atual, mas pra qualquer fase (passada ou atual).
export function computePhaseHistory(platform, refDate = new Date()) {
  ...
  for (let i = 0; i < boundaries.length; i++) {
    ...
    // Saldo Inicial desta fase: gravado na fronteira que ABRIU ela, ou
    // seja, phases[i - 1] (boundaries[i] === phases[i - 1].date). A
    // "Fase 1" (i === 0) nunca tem fronteira anterior, então começa
    // sempre em R$ 0,00.
...
// ATUAL (computeLiveBalance, já com piso em R$ 0,00 e Saldo Inicial da
// fase somado), nunca uma soma das semanas nem das fases anteriores.
...
// por plataforma (cada uma contribui com o Saldo da SUA fase atual,
// já incluindo o Saldo Inicial dela) e NUNCA é afetado pelo filtro de
// datas.
