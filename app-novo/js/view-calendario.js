...
// Painel "🎯 Personalização" montado abaixo do grid de cards de filtro,
// via #cardCustomizationMount. Puramente estético — nenhuma relação com
// colorForLevel()/LEVEL_INFO ou qualquer lógica de ciclo/ordenação já
// existente.
//
// A personalização salva precisa estar carregada no cache ANTES do
// primeiro renderPlatformCards() da sessão (ver refreshPage), senão os
// cards abririam sem cor/marcador na primeira renderização mesmo já
// tendo dado salvo — por isso loadCardCustomization() é aguardado (await)
// logo depois do reset de filtros, antes de montar o painel e antes de
// chamar refreshPage() pela primeira vez.
//
// onSaved (passado a initCardCustomizationPanel) chama só
...
export async function mount(container) {
  ...
  container.innerHTML = `
    ...
    <div id="cardCustomizationMount"></div>
  `;
  ...
  // Bloco L: carrega a personalização salva ANTES do primeiro render dos
...
export function unmount() {
  ...
  if (cardCustomizationCleanup) {
    cardCustomizationCleanup();
    cardCustomizationCleanup = null;
  }
